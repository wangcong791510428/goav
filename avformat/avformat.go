...
func (f *OutputFormat) GetFlags() int {
	return int(f.flags)
}
...
//Open an input stream and read the header.
func AvInterleavedWriteFrame(ps *Context, pkt *avcodec.Packet) int {
	return int(C.av_interleaved_write_frame((*C.struct_AVFormatContext)(unsafe.Pointer(ps)), toCPacket(pkt)))
}
...
func AvIOOpen2(url string, flags int, d **avcodec.Dictionary) (res *AvIOContext, err error) {
	urlStr := C.CString(url)
	defer C.free(unsafe.Pointer(urlStr))

	err = avutil.ErrorFromCode(int(C.avio_open2((**C.AVIOContext)(unsafe.Pointer(&res)), urlStr, C.int(flags), nil, (**C.struct_AVDictionary)(unsafe.Pointer(d)))))
